...
extern "C" int colibri_delta_moe_segment(
    ...
    for (std::int32_t index = 0; index < count; ++index) {
        ...
        }
        {
            void* args[] = {
                ...
                &tokens, &conv_dim, &conv_kernel,
            };
            if (launch(
                    g_kernels.delta_conv,
                    static_cast<unsigned int>(conv_dim), 1, 1, args
                ) != 0) {
                return -3;
            }
        }
        ...
        phase_mark(1);
        // MoE normalization and the router matvec stay on the GPU (the
        // router is a 2 MB read per layer; on the host it costs more than
        // every kernel launch combined). Only 1 KB of logits comes back.
        ...
        phase_mark(2);
        if (g_api.cuMemcpyDtoH(params->hidden_host, mixed, hidden_bytes) != 0
            || g_api.cuMemcpyDtoH(
                   params->normalized_host, moe_normalized, hidden_bytes
               ) != 0
            || g_api.cuMemcpyDtoH(
                   params->logits_host, router_logits,
        ...
        phase_mark(3);
        // Host: top-k over 257 logits, fused Q4 experts, residual.
        ...
        if (g_api.cuMemcpyHtoD(hidden, params->moe_host, hidden_bytes) != 0) {
            return -6;
        }
        phase_mark(5);
    }
    if (std::getenv("COLIBRI_SEG_DEBUG") != nullptr) {
        std::fprintf(
            stderr,
            "[phases ms] mixer=%.3f route=%.3f copies=%.3f topk=%.3f experts=%.3f writeback=%.3f\n",
            g_phase_totals[1], g_phase_totals[2], g_phase_totals[3],
            g_phase_totals[4], g_phase_totals[5], 0.0
        );
