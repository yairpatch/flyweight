...
inline constexpr int32_t kPagedKVPageSize = 64;
inline constexpr int32_t kPagedKVPageShift = 6;
inline constexpr int32_t kPagedKVPageMask = kPagedKVPageSize - 1;
...
FLYWEIGHT_PAGED_HD int32_t paged_kv_physical_page(const int32_t* __restrict__ block_table,
                                                  int32_t token) {
    return block_table[token >> kPagedKVPageShift];
}

// Logical token offset within its physical page (0..63)
FLYWEIGHT_PAGED_HD int32_t paged_kv_page_offset(int32_t token) {
...
FLYWEIGHT_PAGED_HD int64_t paged_kv_row_offset(const int32_t* __restrict__ block_table,
                                               int32_t token,
                                               int32_t kv_head,
                                               int32_t kv_heads,
                                               int32_t capacity,
    ...
    if (block_table == nullptr) {
        return (static_cast<int64_t>(kv_head) * capacity + token) * row_stride;
    }
    const int32_t physical_page = block_table[token >> kPagedKVPageShift];
    const int32_t offset = token & kPagedKVPageMask;
    return ((static_cast<int64_t>(physical_page) * kv_heads + kv_head) * kPagedKVPageSize + offset) *
           row_stride;
}

// Element offset for uncompressed floats/halves (FP32, FP16, BF16)
FLYWEIGHT_PAGED_HD int64_t paged_kv_element_offset(const int32_t* __restrict__ block_table,
                                                  int32_t token,
                                                  int32_t kv_head,
                                                  int32_t kv_heads,
                                                  int32_t head_dim,
...
FLYWEIGHT_PAGED_HD int64_t paged_kv_q8_byte_offset(const int32_t* __restrict__ block_table,
                                                   int32_t token,
                                                   int32_t kv_head,
                                                   int32_t kv_heads,
                                                   int32_t head_dim,
                                                   int32_t capacity) {
    const int32_t blocks = head_dim / 32;
    const int32_t row_bytes = blocks * 34;
...
FLYWEIGHT_PAGED_HD int64_t paged_kv_turbo4_byte_offset(const int32_t* __restrict__ block_table,
                                                       int32_t token,
                                                       int32_t kv_head,
                                                       int32_t kv_heads,
                                                       int32_t head_dim,
                                                       int32_t capacity) {
    const int32_t blocks = head_dim / 32;
    const int32_t row_bytes = blocks * 18;
    return paged_kv_row_offset(block_table, token, kv_head, kv_heads, capacity, row_bytes);
}

// Byte offset for NVFP4 quantized rows (9 bytes per 16 elements)
FLYWEIGHT_PAGED_HD int64_t paged_kv_nvfp4_byte_offset(const int32_t* __restrict__ block_table,
                                                      int32_t token,
                                                      int32_t kv_head,
                                                      int32_t kv_heads,
                                                      int32_t head_dim,
                                                      int32_t capacity) {
    const int32_t blocks = head_dim / 16;
    const int32_t row_bytes = blocks * 9;
    return paged_kv_row_offset(block_table, token, kv_head, kv_heads, capacity, row_bytes);
}
...
class PagedKVPool {
public:
    explicit PagedKVPool(uint32_t total_pages) : total_pages_(total_pages) {
        free_pages_.resize(total_pages);
        for (uint32_t i = 0; i < total_pages; ++i) {
            free_pages_[i] = static_cast<int32_t>(total_pages - 1 - i); // LIFO stack
        }
    }

    [[nodiscard]] int32_t allocate_page() {
    ...
    void free_page(int32_t page) {
        assert(page >= 0 && static_cast<uint32_t>(page) < total_pages_);
        free_pages_.push_back(page);
    }

    [[nodiscard]] uint32_t total_pages() const noexcept { return total_pages_; }
    [[nodiscard]] uint32_t available_pages() const noexcept { return static_cast<uint32_t>(free_pages_.size()); }
    [[nodiscard]] uint32_t allocated_pages() const noexcept { return total_pages_ - available_pages(); }
    [[nodiscard]] size_t token_capacity() const noexcept { return static_cast<size_t>(total_pages_) * kPagedKVPageSize; }
...
public:
    ...
    void ensure_mapped_tokens(int32_t token_count, PagedKVPool& pool) {
        if (token_count <= 0) return;
        const int32_t needed_pages = (token_count + kPagedKVPageSize - 1) >> kPagedKVPageShift;
        while (static_cast<int32_t>(block_table_.size()) < needed_pages) {
            block_table_.push_back(pool.allocate_page());
        }
    }

    // Truncate mappings to token_count, returning excess pages to pool
    void truncate_tokens(int32_t token_count, PagedKVPool& pool) {
        const int32_t needed_pages = token_count <= 0 ? 0 : (token_count + kPagedKVPageSize - 1) >> kPagedKVPageShift;
        while (static_cast<int32_t>(block_table_.size()) > needed_pages) {
    ...
    [[nodiscard]] const int32_t* data() const noexcept { return block_table_.data(); }
    [[nodiscard]] size_t num_pages() const noexcept { return block_table_.size(); }
    [[nodiscard]] int32_t page_at(size_t logical_page) const { return block_table_.at(logical_page); }

    // Direct access to table vector for tests/custom layouts
    std::vector<int32_t>& table() noexcept { return block_table_; }
    [[nodiscard]] const std::vector<int32_t>& table() const noexcept { return block_table_; }
