...
// host kernel backend.
//
// Split out of flyweight_cpu_shim.hpp so translation units that only drive
...
//
// The corpus in flyweight_v2_qwen_kernels.hpp / flyweight_v2_native_kernels.hpp is
// the source of truth for what the runtime computes, and it is launched by
// string name through flyweight_gpu_launch_named. That makes a second, host-side
...
// unqualified min/max overloads the corpus calls (see flyweight_cpu_shim.hpp).
#  ifndef NOMINMAX
#    define NOMINMAX
#  endif
#  include <windows.h>
