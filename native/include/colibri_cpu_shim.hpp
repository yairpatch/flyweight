...
// See colibri_cpu_shim_geometry.hpp for the execution model.
...
using half = colibri::cpu::Half;
using half2 = colibri::cpu::Half2;
using __half = colibri::cpu::Half;
using __half2 = colibri::cpu::Half2;
using __nv_bfloat16 = colibri::cpu::BFloat16;
using nv_bfloat16 = colibri::cpu::BFloat16;

#define blockIdx (::colibri::cpu::t_block_index)
#define threadIdx (::colibri::cpu::t_thread_index)
#define blockDim (::colibri::cpu::t_block_dim)
#define gridDim (::colibri::cpu::t_grid_dim)

inline void __syncthreads() { ::colibri::cpu::t_scheduler->block_barrier(); }
...
#define COLIBRI_VECTOR4(name, element)                       \
...
COLIBRI_VECTOR4(uint4, unsigned int)
COLIBRI_VECTOR4(int4, int)
COLIBRI_VECTOR4(float4, float)
COLIBRI_VECTOR4(ulonglong2_pair, unsigned long long)
#undef COLIBRI_VECTOR4
...
using __nv_fp8_e4m3 = colibri::cpu::__nv_fp8_e4m3;
...
struct __nv_fp4x2_e2m1 {
    ...
    explicit __nv_fp4x2_e2m1(float2 pair)
        : __x(static_cast<unsigned char>(
              colibri::cpu::float_to_e2m1_bits(pair.x) |
              (colibri::cpu::float_to_e2m1_bits(pair.y) << 4))) {}
};
...
// Conversions.
inline float __half2float(colibri::cpu::Half value) {
    return colibri::cpu::half_bits_to_float(value.bits);
}
inline colibri::cpu::Half __float2half(float value) { return colibri::cpu::Half(value); }
inline float __bfloat162float(colibri::cpu::BFloat16 value) {
    return static_cast<float>(value);
}
inline colibri::cpu::BFloat16 __float2bfloat16(float value) {
    return colibri::cpu::BFloat16(value);
}
...
inline T __shfl_sync(unsigned int, T value, int source_lane, int = 32) {
    return colibri::cpu::shuffle_exchange(value, source_lane);
}

template <class T>
inline T __shfl_down_sync(unsigned int, T value, unsigned int delta, int width = 32) {
    const int lane = static_cast<int>(colibri::cpu::t_thread_index.x % colibri::cpu::kWarpSize);
    const int target = lane + static_cast<int>(delta);
    // Out-of-range reads return the lane's own value, as on device.
    return colibri::cpu::shuffle_exchange(value, target < width ? target : lane);
}
...
inline T __shfl_up_sync(unsigned int, T value, unsigned int delta, int = 32) {
    const int lane = static_cast<int>(colibri::cpu::t_thread_index.x % colibri::cpu::kWarpSize);
    const int target = lane - static_cast<int>(delta);
    return colibri::cpu::shuffle_exchange(value, target >= 0 ? target : lane);
}

template <class T>
inline T __shfl_xor_sync(unsigned int, T value, int mask, int = 32) {
    const int lane = static_cast<int>(colibri::cpu::t_thread_index.x % colibri::cpu::kWarpSize);
    return colibri::cpu::shuffle_exchange(value, lane ^ mask);
}

inline void __syncwarp(unsigned int = 0xffffffffu) {
    ::colibri::cpu::t_scheduler->warp_barrier();
}
