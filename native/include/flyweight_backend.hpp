...
// Backend selection for the device interface in flyweight_gpu_driver.h.
//
...
// it changes where flyweight_gpu_* sends the work.
//
// The CPU backend hooks in at two points inside gpu_driver.cpp rather than
// duplicating its ~58 entry points:
//
//   * flyweight_gpu_compile fills g_functions with CPU sentinel handles instead
//     of NVRTC-compiled CUfunctions, so every existing wrapper -- including the
//     ones with hand-tuned grid geometry like flyweight_gpu_q4k_matvec_transposed
...
// Matches flyweight_gpu_driver.h; the library is built with -fvisibility=hidden,
// so selection has to be exported explicitly for the CLI and the tests.
// FLYWEIGHT_V2_STATIC is for the contract executables, which link these sources
// directly: on Windows they neither import nor export, and dllimport on a
// definition is a hard error rather than a warning.
#if defined(FLYWEIGHT_V2_STATIC)
#  define FLYWEIGHT_BACKEND_API
#elif defined(_WIN32)
#  if defined(FLYWEIGHT_V2_BUILD)
#    define FLYWEIGHT_BACKEND_API __declspec(dllexport)
#  else
#    define FLYWEIGHT_BACKEND_API __declspec(dllimport)
#  endif
#else
#  define FLYWEIGHT_BACKEND_API __attribute__((visibility("default")))
#endif
...
// started with FLYWEIGHT_CPU_PROFILE=1. Used to decide which emulated kernels are
// worth hand-writing next.
FLYWEIGHT_BACKEND_API void flyweight_cpu_profile_dump();
