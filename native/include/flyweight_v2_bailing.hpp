...
// this tree: the DeepSeek-V4 router in flyweight_v2_deepseek4.hpp shares the
...
//
// FLYWEIGHT_BAILING_NO_SIMD lets a translation unit that does not link the AVX
// objects (the standalone contract tests) fall back to the scalar path.
#ifndef FLYWEIGHT_BAILING_NO_SIMD
#include "flyweight_gpu_driver.h"
#include "qwen_cpu_kernel.h"
#endif

namespace flyweight::v2::bailing {

// ---------------------------------------------------------------------------
// profiling
// ---------------------------------------------------------------------------
//
// Opt-in phase timers, enabled with FLYWEIGHT_BAILING_PROFILE=1. Off, this costs
...
struct Profile {
    ...
    void report(std::FILE* out) const {
        ...
        if (total <= 0.0) return;
        std::fprintf(out, "[flyweight] bailing profile over %llu positions "
...
inline bool profiling() {
    static const bool on = [] {
        const char* value = std::getenv("FLYWEIGHT_BAILING_PROFILE");
