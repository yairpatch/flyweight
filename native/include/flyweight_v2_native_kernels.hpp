...
inline constexpr char qwen_native_cuda_source[] = R"FLYWEIGHT_CUDA(
...
#define FLYWEIGHT_KQUANT_EMBEDDING(name, decode)                                  \
extern "C" __global__                                                           \
void name(                                                                      \
    const unsigned char* packed, float* output,                                 \
    const int token, const int hidden                                           \
...
FLYWEIGHT_KQUANT_EMBEDDING(qwen_q2k_embedding, q2k_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_q3k_embedding, q3k_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_q4k_embedding, q4k_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_q5k_embedding, q5k_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_q6k_embedding, q6k_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_iq2xxs_embedding, iq2xxs_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_iq3xxs_embedding, iq3xxs_value)
FLYWEIGHT_KQUANT_EMBEDDING(qwen_iq2s_embedding, iq2s_value)
...
R"FLYWEIGHT_CUDA(    const int output_size, const int rows
) {
    const int lane = threadIdx.x & 31;
    const int warp = threadIdx.x >> 5;
    const int output_row = blockIdx.x * 8 + warp;
...
R"FLYWEIGHT_CUDA(        queries[head * head_dim + index] = value;
...
R"FLYWEIGHT_CUDA(            const float softplus_input = decay_logits[token * value_heads + head] + dt_bias[head];
            const float softplus = softplus_input > 20.0f
                ? softplus_input : log1pf(expf(softplus_input));
            decay_scale = expf(decay_coefficients[head] * softplus);
        }
...
// flyweight_v2_bailing.hpp and is checked against it the same way.
...
R"FLYWEIGHT_CUDA(extern "C" __global__
...
void bailing_mla_attention(
    ...
    // mla_attention_absorbed in flyweight_v2_bailing.hpp for the identity.
    //
...
R"FLYWEIGHT_CUDA(            kinv[t] = rsqrtf(key_square + 1.0e-6f);
...
R"FLYWEIGHT_CUDA(                for (int r = 0; r < 4; ++r)
