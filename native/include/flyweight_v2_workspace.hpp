...
constexpr QwenRowsWorkspaceLayout qwen_rows(
    ...
    // Each preserved row is one MTP hand-over. On an arch with hyper-connections
    // that is a whole stream row (hc_count*hidden), because the draft block
    // consumes streams -- sizing this at `hidden` would overrun it by 4x.
    // `hc_count` is 0 on architectures without them, hence the clamp.
    layout.mtp_prompt_hidden =
        builder.add((mtp ? rows + 1 : 0) * (hc_count ? hc_count : 1)
                    * hidden * sizeof(float));
