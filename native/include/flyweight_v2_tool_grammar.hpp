...
inline std::vector<Tool> parse_tool_entries(const flyweight::v2::json::Value& document) {
...
inline std::vector<Tool> parse_specification(const std::string& text) {
    ...
    return parse_tool_entries(flyweight::v2::json::parse(text));
}
...
inline RequestConstraints parse_constraints(const std::string& text) {
    ...
    const auto document = flyweight::v2::json::parse(text);
    if (document.kind == flyweight::v2::json::Kind::Array) {
