...
import ctypes, os, sys, gc, time
from pathlib import Path
from tempfile import TemporaryDirectory
sys.path.insert(0, "/home/yair/Desktop/yair/flyweight")
from tests.dense_gguf_fixture import build_dense_qwen35_gguf
from flyweight.v2 import V2Model, TASK_EVENT_DONE, TASK_EVENT_ERROR
...
def run_task(rt, prompt, n, cancel_after=None):
    ...
    while True:
        for t, tok, kind in rt.engine_step():
            if t != tid: continue
            if kind == TASK_EVENT_ERROR: raise RuntimeError(rt.task_error(tid))
            if kind == TASK_EVENT_DONE: return got
            if kind == 0:
                got += 1
                if cancel_after and got >= cancel_after:
                    rt.task_cancel(tid); cancel_after = None
        # drain until done
        ...
        if mode in ("all", "tasks", "cancel", "lookup"):
            ...
            with V2Model(path) as model, model.native_runtime(**opts) as rt:
                ...
                for i in range(iters):
                    ...
                    gc.collect(); report(mode, i); print("   plen", plen)
main()
