...
class DsparkRuntime:
    def __init__(self, model, context_limit: int):
        ...
        self._check(self._library.flyweight_v2_dspark_runtime_create(
    ...
    def _check(self, status: int) -> None:
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"DSpark error").decode(errors="replace"))

    def inject(self, fused) -> None:
        values = np.ascontiguousarray(fused, dtype=np.float32).reshape(-1)
        self._check(self._library.flyweight_v2_dspark_inject(
            self._handle, values.ctypes.data_as(ctypes.POINTER(ctypes.c_float)), values.size
        ))

    def reset(self) -> None:
        self._check(self._library.flyweight_v2_dspark_runtime_reset(self._handle))

    def cached(self, layer: int, position: int) -> np.ndarray:
        output = np.empty(self._width, dtype=np.float32)
        self._check(self._library.flyweight_v2_dspark_cached(
    ...
    def heads(self, base_logits, hidden, anchor_token: int):
        ...
        self._check(self._library.flyweight_v2_dspark_heads(
            self._model._handle,
    ...
    def attention(self, layer: int, queries, noise_kv):
        ...
        fp = ctypes.POINTER(ctypes.c_float)
        self._check(self._library.flyweight_v2_dspark_attention(
