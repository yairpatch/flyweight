...
from flyweight.v2 import V2Error, _library
...
def matvec(model, name: str, input_vector: np.ndarray, output_size: int) -> np.ndarray:
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"matvec failed").decode(errors="replace"))
    return output
...
def grouped_matvec(
    ...
    library = _library()
    status = library.flyweight_v2_grouped_matvec(
        model._handle, name.encode(), _pointer(input_vector), inputs,
        _pointer(output), rank, groups,
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"grouped matvec failed").decode(errors="replace"))
    return output
...
def head_collapse(
    ...
    *,
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_head(
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"head failed").decode(errors="replace"))
    return pre, output
...
class Deepseek4Runtime:
    ...
    def __init__(self, model, context_limit: int):
        ...
        status = self._library.flyweight_v2_deepseek4_runtime_create(
    ...
    def forward(self, token: int, *, logits: bool = True):
        ...
        status = self._library.flyweight_v2_deepseek4_forward(
            self._handle, int(token), _pointer(out)
        )
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"forward failed").decode(errors="replace")
            )
        return out
    ...
    def capture_layers(self, layers) -> None:
        ...
        status = self._library.flyweight_v2_deepseek4_capture_layers(
            self._handle,
            values.ctypes.data_as(ctypes.POINTER(ctypes.c_uint32)),
            values.size,
        )
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"capture setup failed").decode(errors="replace"))
    ...
    def captured(self):
        ...
        status = self._library.flyweight_v2_deepseek4_captured(
            self._handle, _pointer(output), output.size
        )
        if status < 0:
            raise V2Error((self._library.flyweight_v2_last_error() or b"capture read failed").decode(errors="replace"))
        return output
    ...
    def lm_head(self, hidden):
        ...
        status = self._library.flyweight_v2_deepseek4_lm_head(
            self._handle, _pointer(values), values.shape[0], _pointer(output), output.size
        )
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"LM head failed").decode(errors="replace"))
        return output
    ...
    def prefill(self, tokens) -> None:
        ...
        if not values.size:
            return
        status = self._library.flyweight_v2_deepseek4_prefill(
            self._handle,
            values.ctypes.data_as(ctypes.POINTER(ctypes.c_uint32)),
            values.size,
        )
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"prefill failed").decode(errors="replace")
            )
    ...
    def forward_batch(self, tokens):
        ...
        if not values.size:
            return output
        status = self._library.flyweight_v2_deepseek4_forward_batch(
            self._handle, values.ctypes.data_as(ctypes.POINTER(ctypes.c_uint32)),
            values.size, _pointer(output), output.size,
        )
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"batch forward failed").decode(errors="replace"))
        return output
    ...
    def forward_batch_capture(self, tokens):
        ...
        status = self._library.flyweight_v2_deepseek4_forward_batch_capture(
        ...
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"batch capture failed").decode(errors="replace"))
        return logits, captures

    def snapshot(self):
        handle = ctypes.c_void_p()
        status = self._library.flyweight_v2_deepseek4_snapshot(self._handle, ctypes.byref(handle))
        if status:
            raise V2Error((self._library.flyweight_v2_last_error() or b"snapshot failed").decode(errors="replace"))
    ...
    def reset(self) -> None:
        status = self._library.flyweight_v2_deepseek4_runtime_reset(self._handle)
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"reset failed").decode(errors="replace")
            )
    ...
    def use_gpu(self, device: int = 0) -> None:
        ...
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"gpu enable failed").decode(errors="replace")
            )

    def share_gpu(self, owner: "Deepseek4Runtime") -> None:
        """Use an owner's immutable weights and serialized device workspace."""
        status = self._library.flyweight_v2_deepseek4_runtime_gpu_share(
            self._handle, owner._handle
        )
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"gpu share failed").decode(errors="replace")
            )
    ...
    def attach_gpu(self) -> None:
        ...
        first launch.
        """
        status = self._library.flyweight_v2_deepseek4_runtime_gpu_attach(self._handle)
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"gpu attach failed").decode(errors="replace")
            )
    ...
    def indexer_key(self, layer: int, block: int) -> np.ndarray:
        ...
        """
        from flyweight.v2 import V2Model  # noqa: F401  (documents the source)
        dim = int(self._indexer_dim)
        out = np.zeros(dim, dtype=np.float32)
        status = self._library.flyweight_v2_deepseek4_indexer_key(
            self._handle, int(layer), int(block), _pointer(out), dim
        )
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"indexer key failed").decode(errors="replace")
            )
        return out
    ...
    def info(self) -> dict:
        ...
        status = self._library.flyweight_v2_deepseek4_runtime_info(self._handle, ctypes.byref(value))
        if status:
            raise V2Error(
                (self._library.flyweight_v2_last_error() or b"info failed").decode(errors="replace")
            )
...
class _Deepseek4Snapshot:
    ...
    def restore(self):
        status = self.runtime._library.flyweight_v2_deepseek4_restore(self.runtime._handle, self.handle)
        if status:
...
def half_round_trip(value: float) -> float:
    ...
    return float(_library().flyweight_v2_deepseek4_half_round_trip(float(value)))
...
def gather_block(
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_gather_block(
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"gather block failed").decode(errors="replace"))
...
def visible_keys(
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_visible_keys(
        position, raw_positions, blocks, ratio, window,
        mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), ctypes.byref(visible),
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"visible keys failed").decode(errors="replace"))
    return mask
...
def compress(values: np.ndarray, scores: np.ndarray) -> np.ndarray:
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_compress(
        _pointer(values), _pointer(scores), positions, width, _pointer(output)
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"compress failed").decode(errors="replace"))
    return output
...
def expert_matvec(
    ...
    library = _library()
    status = library.flyweight_v2_expert_matvec(
        model._handle, name.encode(), expert, _pointer(input_vector),
        input_vector.size, _pointer(output), outputs,
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"expert matvec failed").decode(errors="replace"))
    return output
...
def gpu_matvec_check(
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_gpu_matvec_check(
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"gpu matvec check failed").decode(errors="replace"))
...
def indexer_scores(
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_indexer_scores(
        _pointer(queries), _pointer(keys), _pointer(weights),
        queries.shape[0], queries.shape[1], keys.shape[0], _pointer(out),
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"indexer scores failed").decode(errors="replace"))
    return out
...
def top_k_select(scores: np.ndarray, keep: int) -> np.ndarray:
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_top_k(
        _pointer(scores), scores.size, int(keep),
        out.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)),
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"top-k failed").decode(errors="replace"))
    return out
...
def route(
    ...
    *,
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_router(
        _pointer(logits),
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"router failed").decode(errors="replace"))
...
def swiglu(gate: np.ndarray, up: np.ndarray, limit: float) -> np.ndarray:
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_swiglu(
        _pointer(gate), _pointer(up), gate.size, limit, _pointer(output)
    )
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"swiglu failed").decode(errors="replace"))
    return output
...
def rope(
    ...
    *,
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_rope(
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"rope failed").decode(errors="replace"))
    return values
...
def attention(
    ...
    *,
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_attention(
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"attention failed").decode(errors="replace"))
    return output
...
def rms_norm(
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_rms_norm(
        _pointer(values),
    ...
    if status:
        raise V2Error((library.flyweight_v2_last_error() or b"rms norm failed").decode(errors="replace"))
    return output
...
def hyper_connection(
    ...
    *,
    ...
    library = _library()
    status = library.flyweight_v2_deepseek4_hyper_connection(
        _pointer(streams),
        _pointer(fn),
        _pointer(np.ascontiguousarray(scale, dtype=np.float32)),
        _pointer(np.ascontiguousarray(base, dtype=np.float32)),
    ...
    if status:
        message = library.flyweight_v2_last_error() or b"native deepseek4 error"
