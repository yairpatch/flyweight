...
class InferenceService:
    ...
    def __init__(
        ...
        *,
        ...
        # Off unless FLYWEIGHT_TRANSCRIPT_DUMP names a directory. Read once at
        # construction so a live session cannot start paying for it halfway
        # through and produce a dump with a hole in it.
        ...
        *,
        ...
        def _with_metrics(
            ...
            if metrics is not None:
                chunk["flyweight"] = _metrics_payload(metrics)
            return chunk
            ...
            for event in self._generation_events(
                ...
                if isinstance(event, _ReasoningDelta):
                    ...
                    if event.metrics.phase == "tool_call":
                        ...
                        yield {
                            "type": "ping",
                            "flyweight": _metrics_payload(event.metrics),
                        }
                elif isinstance(event, _ToolCallOpen):
                    yield from open_block(
                        "tool_use",
    ...
    def _record_transcript(
        ...
        """Dump this request for the read-before-edit audit, when it is on.

        The prompt is decoded with the special tokens left in, because the
        question the audit answers is what the model saw, and the turn
        boundaries are part of that. Decoding a 40k-token prompt is not free,
        which is the whole reason recording is opt-in.
        """
        if self._transcript is None:
            return
        prompt_text: str | None = None
        try:
            prompt_text = self.generator.tokenizer.decode(
                list(request.prompt_ids), skip_special_tokens=False
            )
        except Exception:
            # A tokenizer that cannot round-trip its own prompt still leaves a
            # usable dump: the client-side half of the check needs no prompt.
            prompt_text = None
...
class FlyweightHTTPServer(ThreadingHTTPServer):
    daemon_threads = True
...
def create_handler(
    ...
    class FlyweightRequestHandler(BaseHTTPRequestHandler):
                ...
                for event in stream:
                    ...
                    if isinstance(event, dict):
                        ...
                        if isinstance(flyweight, dict):
                            ...
                            elapsed = flyweight.get("decode_elapsed_seconds")
                            phase = flyweight.get("phase")
    ...
    return FlyweightRequestHandler
