const STORAGE_KEY = "flyweight.chat.v1";
const SETTINGS_KEY = "flyweight.settings.v1";
const THEME_KEY = "flyweight.theme";
const SIDEBAR_KEY = "flyweight.sidebar";
const API_KEY = "flyweight.api-key";
...
async function sendMessage(promptOverride = null) {
  ...
  try {
    ...
    await readSSE(response.body, (data) => {
      ...
      const generatedTokens = chunk.flyweight?.generated_tokens;
      if (Number.isFinite(generatedTokens)) {
        const now = performance.now();
        firstTokenAt ??= now;
        lastTokenAt = now;
        const serverDecodeSeconds = chunk.flyweight?.decode_elapsed_seconds;
