...
def _select_backend(args: argparse.Namespace) -> str:
    ...
    if selected == "cpu":
        print(
            "[flyweight] running on the CPU backend; decode will be far slower "
            "than a GPU",
            file=sys.stderr,
        )
    return selected
...
def _steady_state_counters(start, end):
    ...
    # Opt-in probe (FLYWEIGHT_ROUTE_RECURRENCE); absent from older runtimes and
...
def _resolve_quant(args: argparse.Namespace) -> None:
    ...
    The loader reads FLYWEIGHT_HF_QUANT, so a choice made here is published as
    ...
    if imatrix is not None and "FLYWEIGHT_HF_IMATRIX" not in os.environ:
        os.environ["FLYWEIGHT_HF_IMATRIX"] = str(imatrix)

    requested = getattr(args, "quant", None)
    if requested and requested != "ask":
        os.environ["FLYWEIGHT_HF_QUANT"] = requested
        return
    if requested is None:
        if os.environ.get("FLYWEIGHT_HF_QUANT"):
            return
    ...
    for _ in range(3):
        ...
        if chosen:
            os.environ["FLYWEIGHT_HF_QUANT"] = chosen
            return
    ...
    os.environ["FLYWEIGHT_HF_QUANT"] = DEFAULT_QUANT
...
def _add_runtime_options(
    ...
    add(
        placement, "--scratch-context", type=int, default=0, metavar="TOKENS",
        help="context for the sequence slots past the first; every slot "
             "reserves its whole context up front, so sizing the side slots "
             "for side traffic (titles, summaries, short subagent calls) "
             "hands the difference to the expert cache. Prompts too long for "
             "a scratch slot are never routed to one. Needs --parallel 2+",
    )
    add(
...
examples:
  ...
  flyweight generate model.gguf --prompt "Explain MoE routing." --max-tokens 128
      one answer on stdout, no server

  flyweight benchmark model.gguf --prompt hi --chat
      prompt and steady-state decode speed as JSON

  flyweight inspect model.gguf
      metadata, config and tensor list as JSON

Run `flyweight COMMAND --help` for one command's full options; each groups
...
def _add_command(
    ...
    parser = commands.add_parser(
        ...
        usage=f"flyweight {usage}", epilog=epilog,
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"flyweight {_version()}",
    )
...
examples:
  ...
  flyweight serve model.gguf --context 65536 --max-tokens 16384
      a longer context window, and longer answers within it

  flyweight serve model.gguf --parallel 2 --cache 4096
      two isolated conversation slots and 4 GiB of host prompt cache

  flyweight serve moe.gguf --expert-mode cpu --cpu-threads 12 \\
...
""",
    ...
    endpoint.add_argument(
        "--api-key", default=os.environ.get("FLYWEIGHT_API_KEY"), metavar="KEY",
        help="require this bearer token on every request; defaults to "
             "FLYWEIGHT_API_KEY, and no key means no authentication",
    )
    endpoint.add_argument(
    ...
    generate = _add_command(
        ...
        epilog="""\
examples:
  flyweight generate model.gguf --prompt "Explain sliding-window attention."

  flyweight generate model.gguf --prompt "Refactor this." \\
      --system "You are a terse code reviewer." --max-tokens 512

  flyweight generate model.gguf --prompt hi --temperature 0.7 --seed 7
...
""",
    ...
    benchmark = _add_command(
        ...
        epilog="""\
examples:
  flyweight benchmark model.gguf --prompt "Explain MoE routing." --chat
      the usual shape: a real chat-formatted prompt, default 128 decode tokens

  flyweight benchmark model.gguf --prompt hi --chat \\
      --context 32768 --warmup 10 --iterations 30

  flyweight benchmark model.gguf --tokens 1,2,3 --cold-cache
...
examples:
  flyweight inspect model.gguf
  flyweight inspect model.gguf | jq .config.architecture
...
""",
    ...
    imatrix = _add_command(
        ...
        epilog="""\
examples:
  flyweight imatrix model.gguf --text calibration.txt --output imatrix.dat

  flyweight imatrix model.gguf --text calibration.txt --max-chunks 64
...
""",
    ...
    probe = _add_command(
        ...
        epilog="""\
examples:
  flyweight probe model.gguf --prompt hi --chat --generate-tokens 8
  flyweight probe model.gguf --token-id 151643
...
""",
    ...
    _add_command(
        commands, "doctor",
        help="check the install and say what to fix",
        description="Report whether this install can serve, and name the fix "
                    "for whatever cannot.",
        usage="doctor",
        epilog="""\
examples:
  flyweight doctor
      before the first serve, or when something failed to start

Checks the Python version, the native runtime library, the build toolchain
behind it, and the NVIDIA driver. Exits non-zero if anything that blocks
serving is missing; a missing driver is reported but does not fail the check,
because --backend cpu serves without one.\
""",
    )
    ...
    audit = _add_command(
        ...
        epilog="""\
examples:
  FLYWEIGHT_TRANSCRIPT_DUMP=/tmp/dump flyweight serve model.gguf
      record every request while a coding harness works

  flyweight transcript-audit /tmp/dump
...
FLYWEIGHT_TRANSCRIPT_PROMPT=0 records a digest instead of the prompt text, which
is much smaller but can only answer the first question.\
""",
    )
    audit.add_argument(
        "directory", type=Path, metavar="DIR",
        help="directory FLYWEIGHT_TRANSCRIPT_DUMP wrote to",
    )
    audit.add_argument(
...
def _validate_runtime_args(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "scratch_context", 0) < 0:
        raise SystemExit("--scratch-context must be non-negative")
    # The native side refuses these too, but a flag that silently did nothing
    # is exactly the failure mode --routed-moe was built to stop repeating.
    if getattr(args, "scratch_context", 0):
        if getattr(args, "parallel_sequences", 1) < 2:
            raise SystemExit(
                "--scratch-context needs --parallel 2 or more; with a single "
                "slot there are no side slots to shrink")
        if getattr(args, "mtp_drafts", 0):
            raise SystemExit(
                "--scratch-context is unavailable under MTP drafting, which "
                "runs a single slot")
        window = getattr(args, "context_window", 0)
        if window and args.scratch_context > window:
            raise SystemExit(
                "--scratch-context must not exceed --context-window")
...
def _runtime_options(args: argparse.Namespace) -> dict[str, object]:
    names = (
        ...
        "parallel_sequences", "prompt_cache_mib", "swa_full", "scratch_context",
...
def _gather_imatrix(args: argparse.Namespace) -> int:
    ...
    _validate_runtime_args(args)
    _select_backend(args)
    os.environ["FLYWEIGHT_IMATRIX"] = "1"
...
def _doctor() -> int:
    ...
    Ordered the way the failures cascade: nothing else matters if the library
    is absent, and the toolchain only matters if it has to be built. Each line
    is a check, a verdict, and -- when it fails -- the one command that fixes
    it, because the whole point is not sending anyone to search.
    """
    from . import native_build

    failures = 0

    def report(name: str, ok: bool | None, detail: str, fix: str = "") -> None:
        nonlocal failures
    ...
    library_ok = False
    if library is None:
        report("native runtime", False, "not built",
               "python -m flyweight.native_build" if source
               else "reinstall from a wheel that bundles the library, or from a clone")
    else:
        stamp = time.strftime("%Y-%m-%d %H:%M",
                              time.localtime(library.stat().st_mtime))
        try:
            # Loading is the only check that proves the ABI matches; a library
            # older than the Python side fails here with a named symbol.
            from .v2 import _library

            _library()
            library_ok = True
            report("native runtime", True, f"{library.name}, built {stamp}")
        except Exception as error:  # noqa: BLE001 - reported, not handled
            report("native runtime", False, f"{library} does not load: {error}",
                   "python -m flyweight.native_build --clean" if source else "")

    if source is None:
        report("sources", None, "no checkout beside this install",
               "fine for a wheel; you cannot rebuild the runtime here")
    else:
        report("sources", True, str(source))
        cmake = shutil.which("cmake")
        if cmake:
            raw = subprocess.run([cmake, "--version"], capture_output=True,
                                 text=True, check=False).stdout.split()
            version = raw[2] if len(raw) > 2 else "?"
            parts = tuple(int(p) for p in re.findall(r"\d+", version)[:2] or [0])
            report("cmake", parts >= (3, 24), version,
                   "" if parts >= (3, 24) else "flyweight needs CMake 3.24+")
        else:
        ...
        if os.name == "nt":
            ...
            report("c++ compiler", bool(compiler), compiler or "none found",
                   "" if compiler else "install a C++20 compiler (GCC 13+ or Clang 16+)")

    # Last, and never fatal: the CPU backend serves without a GPU, so a missing
    # driver is a smaller install, not a broken one. The probe goes through the
    # native library, so without one there is nothing to report but the failure
    # already printed above -- repeating it as a GPU fault reads as two
    # problems where there is one.
    if not library_ok:
        report("nvidia gpu", None, "not probed (needs the native runtime)")
    else:
        try:
    ...
    print("this install can serve" if not failures
          else f"{failures} blocking problem{'s' if failures > 1 else ''} above")
